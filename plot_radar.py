...
rects1 = ax.bar(plot_no, HV, width, color=['turquoise' if i < 12 else 'teal' for i in plot_no],edgecolor='black')
rects2 = ax.bar(w, HV17, width, color=['firebrick' if i < 12 else 'maroon' for i in plot_no],edgecolor='black')
# add some
ax.set_ylabel('Backscatter coefficient [dB]')
ax.set_ylim(-16,0)
ax.set_xlabel('Plot number')
ax.set_title('ALOS PALSAS HV-polarized backscatter measurements',fontsize=14, pad=15)
...
ax.annotate('Logged', xy=(14,-13.5), ha='center', va='center',fontsize=13, weight='bold')
#ax.annotate('', xy=(0.7, -15), xytext=(11.5, -15),
#            arrowprops={'arrowstyle': '<->'})
ax.annotate('Unlogged', xy=(6,-13.5), ha='center', va='center',fontsize=13,weight='bold')

first = plt.legend((rects1[0], rects2[0]), ('2015', '2017'),
          ncol=1,loc='lower left')
ax = plt.gca().add_artist(first)
plt.legend( (rects1[12], rects2[12]), ('2015', '2017'),
          ncol=1,loc='lower right')
fig.savefig(dirpath + '/outputs/site1/radar/ALOS_PALSAR/temporal_variation15_17.pdf', bbox_inches='tight')
